                            ...
                            const bool is_color)
{
  ...
  cimg_forXY(pfm_img,x,y) {
    //std::cout << pfm_img(x,y,0,0) << " ";
    cv_img.at<float>(y,x,0) = -1.f*pfm_img(x,y,0,0);
    //cv_img.at<float>(y,x,0) = pfm_img(x,y,0,0);
    if (is_color) {
...
// for the disparity format simiar to KITTI format
// png do not need extra processing
// https://github.com/lmb-freiburg/dispnet-flownet-docker/issues/6
// We treat the zero-png KITTI pixels as NaN and ignore them in the loss layer. 
cv::Mat ReadPNGImageToCVMat(const string& filename,
                            const int height,
                            const int width,
                            const double scale)
{
  ...
  cv::Mat png_img = cv::imread(filename, CV_LOAD_IMAGE_ANYDEPTH);

  CHECK_EQ(png_img.cols, width) << "Incorrect image width";
  CHECK_EQ(png_img.rows, height) << "Incorrect image height";

  cv::Mat cv_img(cv::Size(width,height), CV_32FC1);

  //std::cout << "convert image...." << std::endl;

  for (int i = 0; i < png_img.rows; ++i){
    for (int j = 0; j < png_img.cols; ++j){
      //if (png_img.at<ushort>(i,j,0) > 0){
       std::cout << png_img.at<ushort>(i,j,0) << " ";
        cv_img.at<float>(i,j,0) = png_img.at<ushort>(i,j,0)/scale;
      /*}
      else{
        cv_img.at<float>(i,j,0) = nanf("");
      }*/
    }
  }
  std::cout << std::endl;
...
public:
    ...
    ImagePair(std::string line)
    {
        ...
        //std::cout << "disp: " << disp_filename << std::endl;
    }
    ~ImagePair()
    {
        if(data) delete data;
    ...
    void read_data()
    {
        ...
        if(has_disp())
        {
            ...
            if (disp_filename.substr(disp_filename.find_last_of(".") + 1) == "pfm"){
              cv_disp = ReadPFMImageToCVMat(disp_filename, ySize, xSize, false);
            }
            else if (disp_filename.substr(disp_filename.find_last_of(".") + 1) == "png"){
              cv_disp = ReadPNGImageToCVMat(disp_filename, ySize, xSize, scale);
            }
            else{
              std::cout << "format not surpport." << std::endl;
            }
        }
        ...
        for (int y = 0; y < height; ++y)
            for (int x = 0; x < width; ++x) {
                ...
                if(has_disp())
                {
                    ...
                    // png no need to do this nan value in loss
                    // https://github.com/lmb-freiburg/dispnet-flownet-docker/issues/6
                    //else if(cv_disp.at<float>(y, x) < 0.01)
                    //    value=std::numeric_limits<short>::max();
                    //else if (cv_disp.at<float>(y, x) > 0)
                    else
                        value=cv_disp.at<float>(y, x) * 32.0; // = flo_data[j]*32.768/1024;
                        // refer https://github.com/lmb-freiburg/flownet2/issues/40
                      //value=cv_disp.at<float>(y, x);
                    //else 
                        // refer to https://github.com/lmb-freiburg/dispnet-flownet-docker/issues/5
                        // the author We converted the PNG groundtruth to PFM, replaced zeroes with NaNs and then used it the same way as e.g. FlyingThings3D.
                    //    value=std::numeric_limits<short>::max();
                }
...
bool EdReadImageToDatum(ImagePair& pair, const int label, const double scale,
     Datum* datum)
{
  pair.scale = scale;
  pair.read_data();
...
void debugData(string value)
{
    ...
    float* disp=new float[xSize*ySize];
...
*/  
    //std::cout << "height: " << datum.height() << std::endl;
    //std::cout << "width: " << datum.width() << std::endl;
    //std::cout << "size: " << datum.data().length() << std::endl;
   ...
   // writePGM("debug.pgm",occ,xSize,ySize);
     writePGM("debug.pgm",disp,xSize,ySize);
    // no sense, the value is changed
    
    // not need
    /*CImg<float> image(xSize,ySize,1);
    cimg_forXY(image,x,y) { image(x,y,0) = disp[y * xSize + x]; }
    image.save("debug.pfm");*/
...
int main(int argc, char** argv) {
  ...
  if (argc < 4 || argc > 9) {
    printf("Convert a set of images to the leveldb format used\n"
        "as input for Caffe.\n"
        "Usage:\n"
        "    convert_imageset_and_disparity LISTFILE DB_NAME"
        " RANDOM_SHUFFLE_DATA[0 or 1] DB_BACKEND[leveldb or lmdb] SCALE[1 or 64 or 256]\n");
    return 1;
  }
  ...
  //for (int line_id = 0; line_id < 1; ++line_id) {
    ...
    if (!EdReadImageToDatum(lines[line_id], 0, scale, &datum)) {
      continue;
    }
