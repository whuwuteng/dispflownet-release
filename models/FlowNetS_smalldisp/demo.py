...
args = [caffe_bin, 'test', '-model', 'tmp/deploy.prototxt',
        '-weights', 'model/flownets_smalldisp.caffemodel',
...
if using_lists:
    ...
    #flownets-pred-0000000.flo
    for (i, target_path) in enumerate(outputs):
        source_path = 'flownets-pred-%07d.flo'%(i)
        subprocess.call(['mv', source_path, dockerize_filepath(target_path)])
else:
    subprocess.call(['mv', 'flownets-pred-0000000.flo', out_files])
